...
export default function PatientProfile({user}) {
  ...
  const gatherProfileInfo = async () => {
    ...
    try {
      const [patResp, apptResp, rxResp] = await Promise.all([
        apiFetch(`${cfmUrl}/patient.cfm?doctorId=${encodeURIComponent(docIden)}&patientId=${encodeURIComponent(currentPatId)}`),
        apiFetch(`${cfmUrl}/appointments.cfm?doctorId=${encodeURIComponent(docIden)}&patientId=${encodeURIComponent(currentPatId)}`),
        apiFetch(`${cfmUrl}/prescriptions.cfm?doctorId=${encodeURIComponent(docIden)}&patientId=${encodeURIComponent(currentPatId)}`)
      ]);
  ...
  const confirmRxDelete = async () => {
    ...
    const docIden = user?.doctor_id || user?.DOCTOR_ID;
    try {
      const resp = await apiFetch(`${cfmUrl}/prescriptions.cfm?doctorId=${docIden}&patientId=${currentPatId}&prescriptionId=${targetId}`, {
          method: 'DELETE'
      });
  ...
  const executeApptCancel = async () => {
    ...
    try {
      ...
      const resp = await apiFetch(`${cfmUrl}/appointments.cfm?doctorId=${docIden}&patientId=${currentPatId}&appointmentId=${cancelId}`, {
          method: 'POST',
          body: JSON.stringify({ 
            action: 'cancel', 
  ...
  if (loadErr) return (
    <Box sx={{ p: 4, maxWidth: '600px', margin: '0 auto' }}>
        ...
        <Button fullWidth variant="plain" onClick={() => routeNav('/search')} sx={{ mt: 1 }}>Return to All Patients</Button>
    </Box>
  );
