...
export default function Appointments({user}) {
    ...
    const fetchAppointmentsData = useCallback(async () => {
        ...
        try {
            ...
            if (isPatient) {
                ...
                endpoint = `${cfmUrl}/patient_appointments.cfm?patientId=${encodeURIComponent(activePatId)}`;
            } else {
                ...
                endpoint = `${cfmUrl}/appointments.cfm?doctorId=${encodeURIComponent(activeDocId)}`;
            }
    ...
    const fetchRemindersData = useCallback(async () => {
        ...
        try {
            const res = await apiFetch(`${cfmUrl}/reminders.cfm?patientId=${encodeURIComponent(activePatId)}`);
            const raw = await res.text();
            let data;
