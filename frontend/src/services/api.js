...
import { AUTH_TOKEN_STORAGE_KEY, apiFetch, cfmUrl } from '../lib/calls';
...
const API_BASE = cfmUrl; // <-- update later
...
export function getPatientInfo(patientId) {
  return fetchData(`${cfmUrl}/patient_self.cfm?patientId=` + encodeURIComponent(patientId));
}

export function getPrescribedMedications(patientId) {
  return fetchData(`${cfmUrl}/patient_medications.cfm?patientId=` + encodeURIComponent(patientId));
}

export function getReminders(patientId) {
  return fetchData(`${cfmUrl}/reminders.cfm?patientId=` + encodeURIComponent(patientId));
}

export function getAppointments(patientId) {
  return fetchData(`${cfmUrl}/patient_appointments.cfm?patientId=` + encodeURIComponent(patientId));
}
...
export function getAssignedDoctors(patientId) {
  return fetchData(`${cfmUrl}/assigned_doctors.cfm?patientId=` + encodeURIComponent(patientId));
}

export function searchDoctors(query) {
  const q = encodeURIComponent(query || '');
  return fetchData(`${cfmUrl}/doctors.cfm?action=search&search_query=` + q);
}

// =================== POST/PUT/DELETE ===================

export function assignDoctor(patientId, doctorId) {
  return axios.post(`${cfmUrl}/doctors.cfm?action=assign&patientId=` + encodeURIComponent(patientId) + '&doctorId=' + encodeURIComponent(doctorId), null, {
...
export function unassignDoctor(patientId, doctorId) {
  return axios.delete(`${cfmUrl}/doctors.cfm?action=unassign&patientId=` + encodeURIComponent(patientId) + '&doctorId=' + encodeURIComponent(doctorId))
    .then((response) => {
      if (response.data?.success === false) {
...
export function postReminder(reminderData) {
  return axios.post(`${cfmUrl}/reminders.cfm?patientId=` + encodeURIComponent(reminderData.patient_id), reminderData, {
    headers: { 'Content-Type': 'application/json' },
  })
    .then(response => {
      if (!Array.isArray(response.data) && response.data?.success === false) {
...
export function deleteReminder(reminderId, patientId) {
  return axios.delete(`${cfmUrl}/reminders.cfm?patientId=${encodeURIComponent(patientId)}&reminderId=${encodeURIComponent(reminderId)}`)
    .then(response => {
      if (response.data?.success === false) {
...
export async function getOptions(){
  try {
    const res = await apiFetch(`/rest/base/options`)
